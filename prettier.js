...
function injectExternalTabs(url) {
    let handle = window.open(url);
    handle.document.querySelector('head').innerHTML += ` <link rel="preconnect" href="https://fonts.googleapis.com">
    ...
    handle.document.querySelector('head').innerHTML += `<link rel='stylesheet' href='https://huodx.github.io/CS31-website-prettifier/prettier-indie.css'>`;
    handle.document.querySelector('head').innerHTML += `<meta charset="UTF-8" /> \n<meta name="viewport" content="width=device-width, initial-scale=1.0" />`;
}
    ...
    pullContentFromSrc(url).then(
        function (content) {
            ...
            document.getElementById('main-panel').querySelectorAll('a').forEach((element) => {
                element.setAttribute('onclick', `javascript:injectExternalTabs('${element.href}');`);
            });
        }
    )
}
